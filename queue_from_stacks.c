...
struct queue_from_stacks* queue_from_stacks_create() {
  struct queue_from_stacks* quack = malloc(sizeof(struct queue_from_stacks));
  quack->s1 = stack_create();
  quack->s2 = stack_create();
  return quack;
}
...
int queue_from_stacks_isempty(struct queue_from_stacks* queue) {
  if (queue == NULL) {
    printf("ERROR: The queue equals null.\n");
    exit(0);
  }
